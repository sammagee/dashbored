<header>
    <div class="bg-white flex h-16 items-center justify-between select-none shadow w-full z-30">
        <div class="flex flex-1 items-stretch">
            <a href="{{ route('dashboard.index') }}" class="bg-black flex items-center justify-center h-16 mr-4 w-16" aria-label="Dashboard">
                <logo class="h-8 text-white"></logo>
            </a>
        </div>
                        ...
                        <div class="relative" @keydown.esc="hide">
                            <button
                                ...
                                :class="{ 'bg-black text-white': open,
                                          'bg-white': !open }"
                                @click="toggle">
                                <span class="font-bold text-inherit">{{ auth()->user()->name }}</span>
